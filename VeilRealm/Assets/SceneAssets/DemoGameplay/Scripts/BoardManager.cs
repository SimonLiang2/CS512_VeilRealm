...
public class BoardManager : MonoBehaviour
{
    ...
    void HandleGameOver(Team winner, Team loser, bool tied, bool flagTaken)
    {
        ...
        if (tied)
        {
            ...
            winnerstats.text = $"Game is a tie! <color={redColor}>Red</color> took <color={redColor}>{redCaptures}</color> pieces!";
            loserstats.text = $"Both teams fought valiantly! <color={blueColor}>Blue</color> took <color={blueColor}>{blueCaptures}</color> pieces!";
        ...
        else
        {
            ...
            gameOverUI.SetActive(true);
            string winnerText = $"<b><color={winnerColor}>{winner} WON</color></b> - Captured <color={winnerColor}>{(winner == Team.RED ? redCaptures : blueCaptures)}</color> pieces.";
            string loserText = $"<b><color={loserColor}>{loser} LOST</color></b> - Captured <color={loserColor}>{(loser == Team.RED ? redCaptures : blueCaptures)}</color> pieces.";
    
            if (flagTaken)
            {
                string reasonText = $"\n<b><color={loserColor}>{loser}'s FLAG</color></b> was captured!";
                reasonwin.text = reasonText;
            }
            else
            {
                string reasonText = $"\n<b><color={loserColor}>{loser}</color></b> had <b><color={neutralColor}>NO MOVABLE PIECES</color></b> left!";
                reasonwin.text = reasonText;
            }
